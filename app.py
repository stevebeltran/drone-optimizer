...
# --- THE MAP RENDERING AND FIXED ZOOM (ONLY CHANGE) ---
fig = go.Figure()
...
if city_boundary_geom is not None and not city_boundary_geom.is_empty:
    ...
    min_lon, min_lat = calls_in_city.geometry.x.min(), calls_in_city.geometry.y.min()
    max_lon, max_lat = calls_in_city.geometry.x.max(), calls_in_city.geometry.y.max()
    center_lon = (min_lon + max_lon)/2
    center_lat = (min_lat + max_lat)/2
    dynamic_zoom = calculate_zoom(min_lon, max_lon)
else:
...
mapbox_config = dict(center=dict(lat=center_lat, lon=center_lon), zoom=dynamic_zoom, style="open-street-map")

fig.update_layout(uirevision=data_signature, mapbox=mapbox_config, margin=dict(l=0,r=0,t=0,b=0), height=800, font=dict(size=18))
